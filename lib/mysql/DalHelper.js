...
class DalHelper {
            ...
            if (whereAnd instanceof Array) {
                whereAnd.forEach((item, i) => {
                    ...
                    if (val instanceof Array) {
                        if (val.length === 1) {
                            whereAnds.push(DalHelper.formatOp(`${table}${columnName}`, `:${pKey}`, op));
                            param[pKey] = val[0];
                        } else {
                        ...
                        whereAnds.push(DalHelper.formatOp(`${table}${columnName.replace(/@/g, '')}`, `${val.replace('@', ':')}`, op));
                    } else if (val === null) {
        ...
        if (whereOr) {
            if (whereOr instanceof Array) {
                whereOr.forEach((item, i) => {
                    ...
                    if (val instanceof Array) {
                        if (val.length === 1) {
                            whereOrs.push(DalHelper.formatOp(`${table}${columnName}`, `:${pKey}`, op));
                            param[pKey] = val[0];
                        } else {
                        ...
                        whereOrs.push(DalHelper.formatOp(`${table}${columnName.replace(/@/g, '')}`, `${val.replace('@', ':')}`, op));
                    } else if (val === null) {
    ...
    static getUpdateSql(option, param, index) {
        ...
        if (option.data instanceof Array) {
            option.data.forEach((item) => {
                ...
                if (key.indexOf('@') === 0) {
                    columnNames.push(DalHelper.formatOp(key.replace(/@/g, ''), val.replace('@', ':'), op));
                } else {
                    columnNames.push(DalHelper.formatOp(key, `:${key}${index}`, op));
                    param[key + index] = val;
                }
            });
        } else {
    ...
    dmls(options) {
        for (let index = 0; index < options.length; index++) {
            ...
            case this.DMLType.UPDATE_INSERT:
                {
                    const supportKeys = ['DMLType', 'table', 'data', 'whereAnd', 'affected', 'identity'];

                    if (Object.keys(option).find((key) => !supportKeys.includes(key))) {
                        return Promise.reject(new Error('存在不支持的option选项'));
                    }
    ...
    static formatWheres(option, param, index) {
        ...
        if (option.whereOrs) {
            option.whereOrs.forEach((whereOr, i) => {
                wheres = wheres.concat(DalHelper.getWheres(null, whereOr, param, `A${index}_${i}`));
            });
        }
        ...
        if (option.ors) {
            ...
            if (option.ors) {
                option.ors.forEach((or, i) => {
                    ors = ors.concat(DalHelper.getWheres(null, or, param, `B${index}_${i}`));
                });
            }

            if (ors.length > 0) {
                whereStr = [...whereStr ? [whereStr] : [], ors.join(' or ')].join(' or ');
            }
        }
    ...
    sql(option) {
        return new DoAction(this.mysql, [option]);
    }
}
